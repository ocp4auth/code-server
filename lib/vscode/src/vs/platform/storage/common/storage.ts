...
export abstract class AbstractStorageService extends Disposable implements IStorageService {
	...
	// NOTE@coder: The rust-analyzer extension stores a value then reloads so
	// this needs to be a promise or the value won't be stored in time before the
	// page reloads.
	store(key: string, value: string | boolean | number | undefined | null, scope: StorageScope, target: StorageTarget): Promise<void> | void {
		...
		return this.withPausedEmitters(() => {
			...
			// Store actual value
			return this.doStore(key, value, scope);
		});
	}
